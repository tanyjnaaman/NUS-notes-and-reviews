...
def evaluation_function(current_cost: float, board: Board, new_piece: Piece) -> tuple((float, float)):
    ...
    def estimateMinSteps(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        ...
        min_steps = max(col_offset, row_offset)
        estimate = min_steps + abs(col_offset - row_offset)
        return estimate

    def estimateMaxSteps(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
    ...
    def chebyshevDistance(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
        estimate = (col_offset + row_offset) - min(col_offset, row_offset)
        return estimate

    def euclideanDistance(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
        estimate = col_offset**2 + row_offset**2
        return estimate
    ...
    def heuristic():
        ...
        for goal in goal_positions:
            estimates.append(chebyshevDistance(current_position, goal))
...
def search_notLimited(board: Board, player: Piece, f): # graph implementation
    
    # initialize
    frontier = customPriorityQueue() 
    explored = {player: 0} # hashtable of piece objects, hashed by their position, mapped to distance g(n)
    numNodesExplored = 0
    ...
    while not frontier.empty():
        ...
        for new_piece in possibleMoves:
            ...
            if (new_piece not in explored) or (cost_to_position < explored[new_piece]):                
                explored[new_piece] = cost_to_position
                new_node = (evaluation, cost_to_position, new_piece)
                frontier.push(new_node)
...
def run_AStar():
    ...
    moves, numNodesExplored, cost = search_notLimited(board, player, f = evaluation_function)
